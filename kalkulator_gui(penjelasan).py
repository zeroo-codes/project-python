...
# BAGIAN 3 : BUAT FUNGSI HANDLING UNTUK WIDGET BUTTON
def klik(tombol):    # 1 buat fungsi klik yang menerima parameter tombol

    # - buat condition, karena nanti widget button nya dibuat menggunakan
    ...
    if tombol == 'c':    # jika argumen tombol yg diterima 'c'   
        ...
        try:    # jika berhasil 
            hasil = eval(entry.get())    # ambil semua elemen yg ada di kotak entry / get() -> hitung expresi mtk nya / eval() -> simpan dlm var 'hasil'  |  sintak di bawah :
            # - get() : untuk mengambil semua elemen( bisa angka / teks ) yg ada di kotak entry
            ...
            entry.insert(tk.END, str(hasil))    # sisipkan / munculkan hasil ekpresi mtk yg sudah dihitung tadi di kotak entry  |  sintak entry insert : .insert(posisi, isi/teks)  ->  menampilkan sesuatu di kotak entry   
...
row_val = 1    # 2 buat variabel row_val sbg baris, dan col_val sebagai kolom untuk mengatur susuan tombol di method grid() nanti.                     |    2 variabel ini nantinya akan terhubung dengan blok kode di bawah              
...
for tombol in tombol_list:    # 3 looping list tombol untuk meringkas kode( agar tidak menulis setiap tombol satu satu )
    ...
    #      A. command=klik() = salah  ->  karena menulis tanda () berarti memanggil. Jadi di sini tkinter hanya menyimpan hasil fungsi nya, bukan referensi fungsi nya
    ...
    #           Sedangkan di sini karena menulis 'command=klik()' tkinter akan mendapat hasil eksekusi fungsi nya, bukan referensi fungsi nya. Jadi tkinter tidak dapat memproses handling nya.
    #           Beda cerita jika fungsi nya mereturn ojbek fungsi / referensi fungsi lainnya
    #      B. command=klik = benar  ->  karena di sini fungsi nya tidak dipanggil, melainkan hanya disimpan tkinter sebagai referensi fungsi
    #         - Bagiamana cara kerja 'command=klik' di tkinter ? karena di sini tidak menulis tanda () berarti fungsi nya tidak dijalankan, yg berarti karena hal itu tkinter akan mendapat referensi fungsi( callback function )
    ...
    col_val += 1    # tambahkan nilai col_val 1 per setiap looping    |    gampangnya blok kode di sini berguna untuk mengatur susunan tombol agar bisa jadi 4 x 4 dengan cara, 1 menambahkan nlai kolom nya setiap kali looping
